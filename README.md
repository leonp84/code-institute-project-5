...
From a business perspective the most important button on this page is of course the ‘Buy this Watch’ button. When users click this a loading animation shows up and after 2 seconds a success message indicates that the watch was added to the user's shopping bag. At the same time the number of items in the bag is automatically updated in the Navigation bar giving clear visual feedback to the user.
...
Successful form submission displays a success message to users, so that they know their message has been sent.
...
<img src="static/assets/images/readme-images/edit-product.jpg" alt="Edit Product" width=800>
<br>
...
<img src="static/assets/images/readme-images/add-new-product.jpg" alt="Add New Product" width=800>
<br>
...
| Expected Outcome                                              | Test Procedure                                                                | Result |
| ------------------------------------------------------------- | ----------------------------------------------------------------------------- | ------ |
| User can browse all products                                  | Click through each product category ensure all products are accessible        | Pass   |
| User can browse products by brand                             | Click on each brand category to ensure products are accessible                | Pass   |
| User can browse products that are on sale                     | Navigate to the sale section to ensure all discounted products are accessible | Pass   |
| Users can click on any product for more detail                | Click on a product to verify detailed information is displayed                | Pass   |
| Users can contact site management regarding specific products | Use the contact form for a specific product to ensure it functions correctly  | Pass   |
...
| Superusers can view site statistics and usage | Log in as a superuser and view the statistics of the admin page to ensure it displays correctly | Pass   |
...
| Users are notified when an order has been successfully paid             | Complete a payment to ensure a notification site (and email) is received | Pass   |
...
| Expected Outcome                                                       | Test Procedure                                                                                       | Result |
| ---------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | ------ |
| Users can pay via credit card                                          | Complete a payment via (Stripe's testing) credit card to ensure the process works correctly          | Pass   |
| Users are notified if their credit card presents errors                | Attempt a payment with an (Stripe's) invalid credit card to ensure an error notification is received | Pass   |
| Users are notified if their orders are successful                      | Complete an order to ensure a success notification page is displayed                                 | Pass   |
...
The Python [Coverage](https://coverage.readthedocs.io/en/7.5.1/) library is showing a total automated test covering of 76% for the website. That takes into account all files for the entire application, though the view.py files by themselves have lower coverage.
...
- The idea to produce an image change and zoom effect when a user hovers over a product (on a product list display page), proved buggy, primarily because the **images tended to load too slow**, negating the impact of the effect. This was corrected by preloading the images upon page load, using the JavaScript `new Image()` functionality.

- Loading product images via fixtures proved uncomplicated, but the images from Tissot (having come for a different source than those of the other brands) had a completely white background, leading to **visual discontinuity** when a user’s shopping bag (or wish list) contained both a Tissot product, and one from another brand. To solve this, I manually edited the Tissot product main images with an image editor, to give the background a similar colour to those of other brands.
...
- **Advanced Search Filters**. Ensuring that all custom filters are taken into account during advanced search, required lots of testing of different filters in isolation. At the end I landed on this solution, which, as far as I can tell, works well:
...
Finally, for sending emails with Django, I needed the following two Config Vars:
...
- For general project inspiration and ideas, the Code Institute PP5 walkthrough, [Boutique Ado](https://github.com/Code-Institute-Solutions/boutique_ado_v1), provided a great starting point.
